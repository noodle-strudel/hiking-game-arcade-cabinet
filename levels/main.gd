...
# methodology: Everything is done together, and the code works even if you
# 	somehow move over the diagonal between chunks even in a single frame. 
	...
	for chunk_z in z_range:
		for chunk_x in x_range:
			...
			if in_bounds:
				...
				_level_grid[chunk_z][chunk_x] =\
						_level_grid[target_chunk_coord_z][target_chunk_coord_x]
			else:
